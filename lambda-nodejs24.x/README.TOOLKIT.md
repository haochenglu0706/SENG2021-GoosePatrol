...
* lambda-nodejs24.x:HelloWorldFunction (nodejs24.x)
* API lambda-nodejs24.x:HelloWorldFunction (nodejs24.x)
...
Invocation parameters, including payloads and request parameters, can be edited either by the `Invoke Locally` command (through the Command Palette or CodeLens) or by editing the `launch.json` file.
